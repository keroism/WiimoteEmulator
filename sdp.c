...
//replace every embedded Wiimote name with the Balance Board name (same length,
//so record sizes and continuation bytes are unaffected)
static void patch_device_name(uint8_t * buf, int32_t len)
{
    static const char wiimote_name[] = "Nintendo RVL-CNT-01";
    static const char board_name[] = "Nintendo RVL-WBC-01";
    const int32_t name_len = sizeof(wiimote_name) - 1;
    int32_t i;

    for (i = 0; i + name_len <= len; i++)
    {
        if (memcmp(buf + i, wiimote_name, name_len) == 0)
        {
            memcpy(buf + i, board_name, name_len);
        }
    }
}
...
{
    patch_device_name(resp1, sizeof(resp1));
    patch_device_name(resp2, sizeof(resp2));
    patch_device_name(attributes, sizeof(attributes));
}
